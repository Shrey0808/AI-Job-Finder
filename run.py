...
def aisubmit():
    if request.method == 'POST':
        # try:
            ...
            no_of_jobs = min(df.size , no_of_jobs)
            df = df.head(no_of_jobs)
            jobs = df.to_dict()

            paginated_jobs, page, total_pages, total_jobs = paginate(jobs)

            return render_template('job_list.html', jobs=paginated_jobs, page=page, total_pages=total_pages, total_jobs=total_jobs, data_fetched=True )        
...
def portal():
    ...
    if request.method == 'POST':
        try:
            portl = str(request.form['jobportal'])
            df = pd.read_csv('job.csv')
            if portl!='Portals':
                df = df[df['source']==portl]
            jobs = df.to_dict()
